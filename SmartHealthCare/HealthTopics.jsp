<%-- 
    Document   : HealthTopics
    Created on : 24 Feb, 2022, 3:14:27 PM
    Author     : HARSHIT DWIVEDI
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <!-- <link rel="stylesheet" href="DEstyle.css"> -->
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <script src="https://kit.fontawesome.com/a076d05399.js" crossorigin="anonymous"></script>
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <link rel="icon"
              href="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ8AbAwyOpA9DR0PrkCif_zdKAfGG0KgAebMA&usqp=CAU">
        <title>Smart HealthCare</title>
        <style>
            * {
                margin: 0%;
                /* padding: 0%; */
            }

            .menubar {
                /* float: right; */
                background-color: black;
                color: white;
                height: 3rem;
                width: 100%;

            }

            .nav {

                padding-top: 15px;
                /* margin-top: 5px; */
                float: right;
                display: flex;

            }

            .nav button {
                background-color: transparent;
                color: white;
                outline: none;
                border: none;
                /* float: right; */
                font-size: medium;
                padding-top: 0px;
                padding-right: 1rem;
            }

            .lang11 {
                /* float: right; */
                margin-right: 2rem;
            }

            .nav button:hover #search {
                display: inline-block;
                background-size: 2rem;
            }

            #search {
                /* padding:5px; */
                outline: none;
            }

            .midhead {
                display: flex;
            }
            .midhead h2{
                font-size: 31px;
                margin-top: 6rem;
                margin-left: 6rem;
                text-align: center;
            }

            .midhead h3{
                margin-top: 7rem;

            }
            .navigate{
                background-color: rgb(121, 185, 60);
                height: 3.1rem;
                padding-top: 7px;
                padding-left: 1rem;
                /* display: flex; */
                /* justify-content: space-evenly; */
            }

            .navbar a {
                text-decoration: none;
                color: black;
                /* margin: 1rem; */
                /* padding-top: 2px; */
                padding: 19px;
                justify-content: space-between;
                font-size: large;
                cursor: pointer;
                font-family: Arial, Helvetica, sans-serif;
            }
            .navigate button{
                outline: none;
                border: 1px solid white;
                color: #0b0c0c;
                float: right;
                margin-right: 1rem;
                padding: 7px;
                margin-top: -25px;
                background-color: transparent;
                box-shadow: 0px 8px 14px 0px rgba(34, 31, 31, 0.5);
                cursor: pointer;
                border-radius: 5px;
            }
            .navigate button:hover{
                background-color: black;
                color: white;
            } 
            .navbar a:hover {
                background-color: sienna;
                padding-bottom: 1.11rem; 
                color: wheat;
                /*transform: scaleY(1.0);*/
            }
            .cards {
                display: flex;
                text-align: center;
                justify-content: space-evenly;
            }

            .card1,
            .card2,
            .card3 {
                border: 1px solid rgb(168, 168, 168);
                padding: 0;
                border-radius: 12px;
            }
            .change{
                border-radius: 12px;
                position: relative;
                left: 40%;
                border: none;
                cursor: pointer;
            }
            #default{
                background-color: rgb(121, 185, 60);
            }
            
            .footer {
                background-color: #101111;
                line-height: 5rem;
                margin-top: 4rem;
            }

            .footer a {
                text-decoration: none;
                color: white;
                font-family: cursive;
            }

            .element {
                display: flex;
                text-decoration: none;
                color: white;
                font-family: cursive;
                padding-left: 24rem;
                /* justify-content: space-around; */
            }

            .element img {
                margin-top: 1.1rem;
            }

            .lstfoot a {
                text-decoration: none;
                color: white;
                font-family: cursive;
            }

            .lstfoot {
                /* padding-left: 10rem; */
                justify-content: space-evenly;
                display: flex;

            }

        </style>
    </head>
    <body>
        <div class="menu">
            <div class="menubar">
                <div class="nav">
                    <button id="btn"><i class="fa fa-search" aria-hidden="true"></i> <input type="text" name="search"
                                                                                            id="search" placeholder="Search your text" hidden> </button>
                    <p class="lang11">Language: <select name="Lang" id="lang">
                            <option name="Lang" value="English">English</option>
                            <option name="Lang" value="Hindi">Hindi</option>
                            <option name="Lang" value="Spanish">Spanish</option>
                            <option name="Lang" value="French">French</option>
                        </select>
                    </p>
                </div>
            </div>
            <div class="midhead">
                <img style="margin-top: 4rem; margin-left: 5px;" src="1.jpg" width="180px" height="160px">
                <h3
                    style="font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif;  color: rgb(56, 116, 184); font-size: x-large;">
                    Smart Health Care</h3>
                <h2  style="font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif;  color: rgb(207, 138, 59);" >Explore the future of health</h2>
            </div><br>
            <div class="navigate">
                <div class="navbar">
                    <a class="links" href="home.jsp"><i class="fa fa-home" aria-hidden="true" style="font-size: xx-large;"></i></a>
                    <a class="links" href="HealthTopics.jsp" id="topics">Health Topics</a>
                    <a class="links" href="emerge.jsp">Emergencies</a>
                    <!-- <a href="login.html">Sign Up/Log In</a> -->
                    <a class="links" href="aboutUS.jsp">About Us</a></div>
                <a href="loggin.jsp"><button class="btn11"> SIGN IN </button></a>
                <a href="signUP.html"><button class="btn11">SIGN UP</button></a>
            </div><br> <br>
            <button type="submit" class="change" id="default"><a href="HealthTopics.jsp" style="text-decoration:none; color: black"><h3 style="text-align: center;">Wellness Topics</h3></a></button>
            <button type="submit" class="change" id="commitChange"><a href="Reviews.jsp" style="text-decoration:none; color: black"><h3 style="text-align:center;">Health Reviews</h3></a></button><br><br>
            <br>
            <div class="cards">
                <div class="card1" style="width: 18rem;">
                    <img src="https://post-staging.healthline.com/wp-content/uploads/2021/10/nutrition.jpg"
                         class="card-img-top" width="289px" height="150px">
                    <div class="card-body">
                        <h3 class="card-title">Nutrition</h3>
                        <br>
                        <p class="card-text">Healthy eating in real life</p>
                    </div>
                </div>
                <div class="card2" style="width: 18rem;">
                    <img src="https://post-staging.healthline.com/wp-content/uploads/2021/10/mental_health.jpg"
                         class="card-img-top" width="288px" height="150px">
                    <div class="card-body">
                        <h3 class="card-title">Mental Health</h3>
                        <br>
                        <p class="card-text">Resources to care for your mental and emotional well being</p><br>
                    </div>
                </div>
                <div class="card3" style="width: 18rem;">
                    <img src="https://post-staging.healthline.com/wp-content/uploads/2021/10/Fitness.jpg"
                         class="card-img-top" width="288px" height="150px">
                    <div class="card-body">
                        <h3 class="card-title">Fitness</h3>
                        <br>
                        <p class="card-text">Fitness that fits you</p>
                    </div>
                </div>
            </div>
            <br> <br>
            <div class="cards">
                <div class="card1" style="width: 18rem;">
                    <img src="https://post-staging.healthline.com/wp-content/uploads/2021/10/Parenthood.jpg"
                         class="card-img-top" width="289px" height="150px">
                    <div class="card-body">
                        <h3 class="card-title">Parenthood</h3>
                        <br>
                        <p class="card-text">Take care of you so you can take care of them</p>
                    </div>
                </div>
                <div class="card2" style="width: 18rem;">
                    <img src="https://post-staging.healthline.com/wp-content/uploads/2021/10/Sleep.jpg" class="card-img-top"
                         width="288px" height="150px">
                    <div class="card-body">
                        <h3 class="card-title">Sleep</h3>
                        <br>
                        <p class="card-text">Your biggest ally in your pursuit of getting healthier sleep</p><br>
                    </div>
                </div>
                <div class="card3" style="width: 18rem;">
                    <img src="https://post.healthline.com/wp-content/uploads/2022/01/reviews_hubofhub.png"
                         class="card-img-top" width="288px" height="150px">
                    <div class="card-body">
                        <h3 class="card-title"><a href="" style="text-decoration:none; color: black;">Product Reviews</a></h3>
                        <br>
                        <p class="card-text">Simplify your search for health products with experts reviews</p>
                    </div>
                </div>
            </div><br>
            <div class="footer">
                <div class="element"><img
                        src="https://www.freepnglogos.com/uploads/twitter-logo-png/twitter-bird-symbols-png-logo-0.png"
                        width="50px" height="50px"><a href="#">@HealthCare</a>
                    <div class="element">
                        <img src="data:image/png;base64,"
                             width="40px" height="40px"><a href="#">Get
                            Email Updates</a>
                    </div>
                </div>
                <hr style="color: white;">
                <div class="lstfoot">
                    <div class="lstfoot1"><a href="#">Copyright©2022 HealtCare</a></div>
                    <div class="lstfoot2"><a href="aboutUS.jsp">Privacy Policy</a></div>
                    <div class="lstfoot3"><a href="aboutUS.jsp">Terms of Use</a></div>
                </div>
            </div>

            <script>
                $(document).ready(function () {
                    $('#commitChange').click(function () {
                        $('#commitChange').css('background-color', 'rgb(121, 185, 60)');
                        $('#default').css('background-color', 'rgba(201, 200, 200, 0.5)');
                    });
                    $('#default').click(function () {
                        $('#default').css('background-color', 'rgb(121, 185, 60)');
                        $('#commitChange').css('background-color', 'rgba(201, 200, 200, 0.5)');
                    })
                })
            </script>
    </body>
</html>
