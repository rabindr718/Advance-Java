<%-- 
    Document   : Reviews
    Created on : 26 Feb, 2022, 1:49:40 AM
    Author     : HARSHIT DWIVEDI
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <script src="https://kit.fontawesome.com/a076d05399.js" crossorigin="anonymous"></script>
        <link rel="icon"
              href="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ8AbAwyOpA9DR0PrkCif_zdKAfGG0KgAebMA&usqp=CAU">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">

        <title>Smart HealthCare</title>
        <style>
            * {
                margin: 0%;
                /* padding: 0%; */
            }

            .menubar {
                /* float: right; */
                background-color: black;
                color: white;
                height: 3rem;
                width: 100%;

            }

            .nav {

                padding-top: 15px;
                /* margin-top: 5px; */
                float: right;
                display: flex;

            }

            .nav button {
                background-color: transparent;
                color: white;
                outline: none;
                border: none;
                /* float: right; */
                font-size: medium;
                padding-top: 0px;
                padding-right: 1rem;
            }

            .lang11 {
                /* float: right; */
                margin-right: 2rem;
            }

            .nav button:hover #search {
                display: inline-block;
                background-size: 2rem;
            }

            #search {
                /* padding:5px; */
                outline: none;
            }

            .midhead {
                display: flex;
            }
            .midhead h2{
                font-size: 31px;
                margin-top: 6rem;
                margin-left: 6rem;
                text-align: center;
            }

            .midhead h3{
                margin-top: 7rem;

            }
            .navigate{
                background-color: rgb(121, 185, 60);
                height: 3.1rem;
                padding-top: 7px;
                padding-left: 1rem;
                /* display: flex; */
                /* justify-content: space-evenly; */
            }

            .navbar a {
                text-decoration: none;
                color: black;
                /* margin: 1rem; */
                /* padding-top: 2px; */
                padding: 19px;
                justify-content: space-between;
                font-size: large;
                cursor: pointer;
                font-family: Arial, Helvetica, sans-serif;
            }
            .navigate button{
                outline: none;
                border: 1px solid white;
                color: #0b0c0c;
                float: right;
                margin-right: 1rem;
                padding: 7px;
                margin-top: -25px;
                background-color: transparent;
                box-shadow: 0px 8px 14px 0px rgba(34, 31, 31, 0.5);
                cursor: pointer;
                border-radius: 5px;
            }
            .navigate button:hover{
                background-color: black;
                color: white;
            } 
            .navbar a:hover {
                background-color: sienna;
                padding-bottom: 1.11rem; 
                color: wheat;
                /*transform: scaleY(1.0);*/
            }
            .change{
                border-radius: 12px;
                position: relative;
                left: 40%;
                border: none;
                cursor: pointer;
            }
            #commitChange{
                background-color: rgb(121, 185, 60);
            }
            .desc img{
                position: relative;
            }
            .desc h1{
                position: absolute;
                top: 29rem;
                left: 42%;
                color: black;
                font-size: xx-large;
                font-family: cursive;
                font-weight: 700;
            }
            .desc h3{
                position: absolute;
                top: 34rem;
                left: 12%;
                color: black;
                font-size: x-large;
                font-family: cursive;

            }
            .footer {
                background-color: #101111;
                line-height: 5rem;
                margin-top: 4rem;
            }

            .footer a {
                text-decoration: none;
                color: white;
                font-family: cursive;
            }

            .element {
                display: flex;
                text-decoration: none;
                color: white;
                font-family: cursive;
                padding-left: 24rem;
                /* justify-content: space-around; */
            }

            .element img {
                margin-top: 1.1rem;
            }

            .lstfoot a {
                text-decoration: none;
                color: white;
                font-family: cursive;
            }

            .lstfoot {
                /* padding-left: 10rem; */
                justify-content: space-evenly;
                display: flex;

            }
            .descc{
                display: flex;
            }
            .desc1{
                margin-left: 4rem;

            }
            /*            .subdesc1{
                            display: inline;
                        }*/
            .subdesc2{
                display: block;
                margin-left: 7px;
            }
            .subdesc2 a{
                text-decoration: none;
                color: black;
                font-size: x-large;
            }
            .subdesc2 a:hover{
                color:	#FF1493;
            }
            .parent{
                display: flex;
            }
            #rightdesc{
                position: absolute;
                left:  50rem;
            }
            .parent{
                font-family: centaur;
            }
            .viewAll{
                padding: 5px;
                font-family: cursive;
                cursor: pointer;
                border: none;
                border-radius: 5px;
                position: relative;
                left: 44rem;
            }
            .viewAll:hover{
                background-color: brown;
                color: white;
            }
            .checked {
                color: orange;
            }
            /*            #hidedesc{
                            display: none;
                        }*/
        </style>
    </head>
    <body>
        <div class="menu">
            <div class="menubar">
                <div class="nav">
                    <button id="btn"><i class="fa fa-search" aria-hidden="true"></i> <input type="text" name="search"
                                                                                            id="search" placeholder="Search your text" hidden> </button>
                    <p class="lang11">Language: <select name="Lang" id="lang">
                            <option name="Lang" value="English">English</option>
                            <option name="Lang" value="Hindi">Hindi</option>
                            <option name="Lang" value="Spanish">Spanish</option>
                            <option name="Lang" value="French">French</option>
                        </select>
                    </p>
                </div>
            </div>
            <div class="midhead">
                <img style="margin-top: 4rem; margin-left: 5px;" src="1.jpg" width="180px" height="160px">
                <h3
                    style="font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif;  color: rgb(56, 116, 184); font-size: x-large;">
                    Smart Health Care</h3>
                <h2  style="font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif;  color: rgb(207, 138, 59);" >Your Health is our priority</h2>
            </div><br>
            <div class="navigate">
                <div class="navbar">
                    <a class="links" href="index.html"><i class="fa fa-home" aria-hidden="true" style="font-size: xx-large;"></i></a>
                    <a class="links" href="HealthTopics.jsp" id="topics">Health Topics</a>
                    <a class="links" href="emerge.jsp">Emergencies</a>
                    <!-- <a href="login.html">Sign Up/Log In</a> -->
                    <a class="links" href="aboutUS.jsp">About Us</a></div>
                <a href="loggin.jsp"><button class="btn11"> SIGN IN </button></a>
                <a href="signUP.html"><button class="btn11">SIGN UP</button></a>
            </div><br>
            <button type="submit" class="change" id="default"><a href="HealthTopics.jsp" style="text-decoration:none; color: black"><h3 style="text-align: center;">Wellness Topics</h3></a></button>
            <button type="submit" class="change" id="commitChange"><a href="Reviews.jsp" style="text-decoration:none; color: black"><h3 style="text-align:center;">Health Reviews</h3></a></button><br><br>

            <div class="desc">
                <img src="https://koreaprofessional.com/wp-content/uploads/2020/05/Healthcare3.jpg" width="100%" height="400px">
                <h1>Product Reviews</h1>
                <h3>Simplify your search for health products with our evidence-based reviews and brand comparisons.</h3>
            </div>
            <br>
            <div class="desc1">
                <h3 style="font-size:x-large; font-family: sans-serif">Most Popular</h3>
                <br> <hr style="width:95%; text-align: center;"><br>
                <div class="parent">
                    <div class="descc">
                        <div class="subdesc1">
                            <img src="https://post.healthline.com/wp-content/uploads/2020/02/250481_Prenatal_Vitamins_1296x728.png" width="150px" height="90px"></div>
                        <div class="subdesc2"><p>PREGNANCY</p> <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                            <a href="#">The Best Prenatal Vitamins for a Healthy Pregnancy</a>
                        </div>&nbsp;
                    </div>
                    <div class="descc" id="rightdesc">
                        <div class="subdesc1">
                            <img src="https://post.healthline.com/wp-content/uploads/2021/04/1080051-The-7-Best-At-Home-Food-Sensitivity-Tests-of-2021-1296x728-Header-ecafa7.jpg" width="150px" height="100px"></div>
                        <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span>
                            <span class="fa fa-star"></span><br>
                            <a href="#">The 5 Best At-Home Food Sensitivity Tests of 2022</a>
                        </div>

                    </div>
                </div>
            </div><br>
            <div class="desc1">
                <h3 style="font-size:x-large; font-family: sans-serif">Editor's Picks</h3>
                <br> <hr style="width:95%; text-align: center;"><br>
                <div class="parent">
                    <div class="descc">
                        <div class="subdesc1">
                            <img src="https://post.healthline.com/wp-content/uploads/2021/04/hydrow-rowing-machine-1296x728-header.jpg" width="150px" height="90px"></div>
                        <div class="subdesc2"> <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                            <a href="#">Hydrow Review: We Tried The Tesla of Rowing Machines</a>
                        </div>&nbsp;
                    </div>
                    <div class="descc" id="rightdesc">
                        <div class="subdesc1">
                            <img src="https://post.healthline.com/wp-content/uploads/2021/04/960806-The-10-Best-Workout-Mats-for-Any-Exercise-1296x728-Header-c0dcdf.jpg" width="150px" height="100px"></div>
                        <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span><br>
                            <a href="#">The 10 Best Workout Mats for Any Exercise</a>
                        </div>

                    </div>
                </div>
            </div><br>
            <div class="desc1">
                <h3 style="font-size:x-large; font-family: sans-serif">VITAMINS AND SUPPLEMENTS</h3>
                <br> <hr style="width:95%; text-align: center;"><br>
                <div class="parent">
                    <div class="descc">
                        <div class="subdesc1">
                            <img src="https://post.healthline.com/wp-content/uploads/2021/06/1016631-CLONE-V2-3152021-The-15-Best-Multivitamins-for-Men-1296x728-Header-81b9bf.jpg" width="150px" height="90px"></div>
                        <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                            <a href="#">The 10 Best Multivitamins for Men, According <br> to a Dietitian</a>
                        </div>&nbsp;
                    </div>
                    <div class="descc" id="rightdesc">
                        <div class="subdesc1">
                            <img src="https://post.healthline.com/wp-content/uploads/2021/10/1649157-%E2%80%93-9-of-the-Best-Multivitamins-for-Womens-Health-1296x728-Header-81b9bf.jpg" width="150px" height="90px"></div>
                        <div class="subdesc2"><p>WOMENSVITAMINS</p><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                            <a href="#">The 9 Best Multivitamins for Women’s Health</a>
                        </div>

                    </div>
                </div>
            </div>
            <br>

            <button type="submit" class="viewAll" id='view'>Show more</button><br>
            <div id='hidedesc'>
                <div class="desc1">
                    <h3 style="font-size:x-large; font-family: sans-serif">SLEEP</h3>
                    <br> <hr style="width:95%; text-align: center;"><br>
                    <div class="parent">
                        <div class="descc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2020/10/688671-The-6-Best-Extra-Firm-Mattresses-1296x728-Header-81b9bf.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span>
                            <span class="fa fa-star"></span><br>
                                <a href="#">The Best Mattresses for Lower Back Pain</a>
                            </div>&nbsp;
                        </div>
                        <div class="descc" id="rightdesc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2019/09/POST-151002-10_of_the_Best_Pillows_for_Neck_Pain_and_How_to_Choose_One-1296x728-header-1296x728.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                                <a href="#">9 of the Best Pillows for Neck Pain and How to Choose<br> One</a>
                            </div>

                        </div>
                    </div>
                </div>
                <br>
                <div class="desc1">
                    <h3 style="font-size:x-large; font-family: sans-serif">Nutrition</h3>
                    <br> <hr style="width:95%; text-align: center;"><br>
                    <div class="parent">
                        <div class="descc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2021/04/755354-The-Top-6-Meal-Delivery-Services-for-Weight-Loss-1296x728-Header-fdbb44.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span><br>
                                <a href="#">Best Meal Delivery Services for Weight Loss</a>
                            </div>&nbsp;
                        </div>
                        <div class="descc" id="rightdesc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2020/12/795536-The-6-Best-Gluten-Free-Meal-Delivery-Services-in-2020-1296x728-Header-f5d7d3.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                                <a href="#">The 5 Best Gluten-Free Meal Delivery Services for 2021</a>
                            </div>

                        </div>
                    </div>
                </div>
                <br>
                <div class="desc1">
                    <h3 style="font-size:x-large; font-family: sans-serif">Fitness</h3>
                    <br> <hr style="width:95%; text-align: center;"><br>
                    <div class="parent">
                        <div class="descc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2021/06/1256605-The-5-Best-NordicTrack-Treadmills-of-2021-1296x728-Header-e0edef.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span><br>
                                <a href="#">The 5 Best NordicTrack Treadmills of 2022</a>
                            </div>&nbsp;
                        </div>
                        <div class="descc" id="rightdesc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2021/06/1094480-The-9-Best-Recumbent-Exercise-Bikes-You-Can-Buy-In-2021-1296x728-Header-81b9bf.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                                <a href="#">The 9 Best Recumbent Exercise Bikes You Can Buy In<br> 2022</a>
                            </div>
                        </div>
                    </div>
                </div><br>
                <div class="desc1">
                    <h3 style="font-size:x-large; font-family: sans-serif">SkinCare</h3>
                    <br> <hr style="width:95%; text-align: center;"><br>
                    <div class="parent">
                        <div class="descc">
                            <div class="subdesc1">
                                <img src="11.jpeg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star"></span><br>
                                <a href="#">13 Powerful Home Remedies for Acne and Hair fall </a>
                            </div>&nbsp;
                        </div>
                        <div class="descc" id="rightdesc">
                            <div class="subdesc1">
                                <img src="https://post.healthline.com/wp-content/uploads/2021/09/1493443-15-Best-Eye-Creams-for-Dark-Circles-According-to-Dermatologists-1296x728-Header-ecafa7.jpg" width="150px" height="90px"></div>
                            <div class="subdesc2"><span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span>
                            <span class="fa fa-star checked"></span><br>
                                <a href="#">15 Best Eye Creams for Dark Circles,<br> According to Dermatologists</a>
                            </div>
                        </div>
                    </div>
                </div><br>
                <button type="submit" class="viewAll" id='show'>Show Less</button><br>
            </div>
            <div class="footer">
                <div class="element"><img
                        src="https://www.freepnglogos.com/uploads/twitter-logo-png/twitter-bird-symbols-png-logo-0.png"
                        width="50px" height="50px"><a href="#">@HealthCare</a>
                    <div class="element">
                        <img src="data:image/png;base64,"
                             width="40px" height="40px"><a href="#">Get
                            Email Updates</a>
                    </div>
                </div>
                <hr style="color: white;">
                <div class="lstfoot">
                    <div class="lstfoot1"><a href="#">Copyright©2022 HealtCare</a></div>
                    <div class="lstfoot2"><a href="aboutUS.jsp">Privacy Policy</a></div>
                    <div class="lstfoot3"><a href="aboutUS.jsp">Terms of Use</a></div>
                </div>
            </div>
            <script>
                $(document).ready(function () {
                    $('#commitChange').click(function () {
                        $('#commitChange').css('background-color', 'rgb(121, 185, 60)');
                        $('#default').css('background-color', 'rgba(201, 200, 200, 0.5)');
                    });
                    $('#default').click(function () {
                        $('#default').css('background-color', 'rgb(121, 185, 60)');
                        $('#commitChange').css('background-color', 'rgba(201, 200, 200, 0.5)');
                    })

                    $('#hidedesc').hide();
                    $('#view').click(function () {
                        $('#hidedesc').show(300);
                        $('#show').show();
                        $('#view').hide();
                    });
                    $('#show').click(function () {
                        $('#hidedesc').hide(150);
                        $('#view').show();
                        $(this).hide();
                    });
                })
            </script>
    </body>
</html>
